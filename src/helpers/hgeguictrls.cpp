...
hgeGUIText::hgeGUIText(int _id, float x, float y, float w, float h, hgeFont2 *fnt)
{
	id=_id;
	bStatic=true;
	bVisible=true;
...
void hgeGUIText::SetText(const wchar_t *_text)
{
	wcscpy(text, _text);
}

void hgeGUIText::printf(const wchar_t *format, ...)
{
	vswprintf(text, format, (char *)&format+sizeof(format));
}
...
hgeGUIListbox::hgeGUIListbox(int _id, float x, float y, float w, float h, hgeFont2 *fnt, DWORD tColor, DWORD thColor, DWORD hColor)
{
	id=_id;
	bStatic=false;
	bVisible=true;
...
int hgeGUIListbox::AddItem(wchar_t *item)
{
	hgeGUIListboxItem *pItem=pItems, *pPrev=0, *pNew;

	pNew = new hgeGUIListboxItem;
	memcpy(pNew->text, item, min(sizeof(pNew->text), wcslen(item)+1));
...
wchar_t *hgeGUIListbox::GetItemText(int n)
{
	int i;
