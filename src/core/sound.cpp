...
HEFFECT CALL HGE_Impl::Effect_Load(const wchar_t *filename, DWORD size)
{
		...
		if(!hs) {
			...
			if(hstrm) {
				...
				if(!buffer)
				{
					...
					_PostError(L"Can't create sound effect: Not enough memory");
				}
				else
				{
					...
					BASS_StreamFree(hstrm);
					if(!hs)	_PostError(L"Can't create sound effect");
				}
			}
		}
...
HMUSIC CALL HGE_Impl::Music_Load(const wchar_t *filename, DWORD size)
{
	void *data;
	DWORD _size;
	HMUSIC hm;
	...
	if(hBass)
	{
		...
		if(!hm)	_PostError(L"Can't load music");
		if(!size) Resource_Free(data);
		return hm;
	}
	else return 0;
...
HSTREAM CALL HGE_Impl::Stream_Load(const wchar_t *filename, DWORD size)
{
	void *data;
	DWORD _size;
	HSTREAM hs;
	...
	if(hBass)
	{
		...
		if(!hs)
		{
			_PostError(L"Can't load stream");
			if(!size) Resource_Free(data);
			return 0;
		}
		if(!size)
...
bool HGE_Impl::_SoundInit()
{
	...
	hBass=LoadLibrary(L"bass.dll");
	if (!hBass)
	{
		_PostError(L"Can't load BASS.DLL");
		return false;
	}
	...
	{
		_PostError(L"Incorrect BASS.DLL version");
		return false;
	}
	...
	{
		System_Log(L"BASS Init failed, using no sound");
		BASS_Init(0,nSampleRate,0,hwnd,NULL);
		bSilent=true;
	}
	else
	{
		System_Log(L"Sound Device: %s",BASS_GetDeviceDescription(1));
		System_Log(L"Sample rate: %ld\n", nSampleRate);
	}
