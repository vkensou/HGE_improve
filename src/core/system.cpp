...
bool CALL HGE_Impl::System_Start()
{
	...
	if(!hwnd)
	{
		_PostError(L"System_Start: System_Initiate wasn't called");
		return false;
	}
...
void CALL HGE_Impl::System_SetStateString(hgeStringState state, const wchar_t *value)
{
	...
	switch(state)
	{
		...
		case HGE_TITLE:			wcscpy(szWinTitle,value);
								...
								break;
		case HGE_INIFILE:		if(value) wcscpy(szIniFile,Resource_MakePath(value));
								else szIniFile[0]=0;
								break;
		case HGE_LOGFILE:		if(value)
								{
									wcscpy(szLogFile,Resource_MakePath(value));
...
void CALL HGE_Impl::System_Snapshot(const wchar_t *filename)
{
	...
	if(!filename)
	{
		...
		swprintf(tempname, L"shot%03d.bmp", i);
...
HGE_Impl::HGE_Impl()
{
	...
	int i;
	for(i=wcslen(szAppPath)-1; i>0; i--) if(szAppPath[i]==L'\\') break;
	szAppPath[i+1]=0;
}
