...
//要求目标是空指针
inline int W2M(char ** dest,const wchar_t * src)
{
	int dlen,slen=wcslen(src) + 1;
	dlen = WideCharToMultiByte(CP_ACP,NULL,src,slen,NULL,0,NULL,NULL);
	if(dlen>0)
	{
		*dest = new char[dlen];
		WideCharToMultiByte(CP_ACP, 0, src, slen, *dest, dlen, NULL, NULL);	
	}
	else
		*dest = 0;
	return dlen;
...
inline int M2W(wchar_t ** dest,const char * src)
{
	int dlen,slen=strlen(src) + 1;
	dlen = MultiByteToWideChar(CP_ACP,0,src,slen,NULL,0);
	if(dlen>0)
	{
		*dest = new wchar_t[dlen];
		MultiByteToWideChar(CP_ACP, 0, src, slen, *dest, dlen);
	}
	else
		*dest = 0;
	return dlen;
...
public:
	...
	//获取服从正态分布的随机数，参数为平均值和标准差
	virtual float		CALL	Random_Guss(float avrgvalue ,float standdeviation) = 0;

	virtual float		CALL	Timer_GetTime() = 0;
	virtual float		CALL	Timer_GetDelta() = 0;
	virtual int			CALL	Timer_GetFPS() = 0;

	virtual HEFFECT		CALL	Effect_Load(const wchar_t *filename, DWORD size=0) = 0;
	...
	virtual HMUSIC		CALL	Music_Load(const wchar_t *filename, DWORD size=0) = 0;
	...
	virtual HSTREAM		CALL	Stream_Load(const wchar_t *filename, DWORD size=0) = 0;
	...
	//virtual int CALL Gfx_GetD3d9Device() = 0;
	...
	virtual void		CALL	Gfx_RenderDone() = 0;
	...
	virtual bool		CALL	Shader_CreatePixelShader( const wchar_t *psFileName, const char *mainFunc, const char *virsion, HPIXSHADER* pixHandle, HCONSTTABLE* tableHandle ) = 0;
	virtual bool		CALL	Shader_SetPixelShader( HPIXSHADER pixHandle ) = 0;
	virtual void		CALL	Shader_FreePixelShader( HPIXSHADER pixHandle ) = 0;
	virtual void		CALL	Shader_FreeConstTable( HCONSTTABLE tableHandle ) = 0;
