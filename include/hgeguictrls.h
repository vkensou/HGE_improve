...
public:
	virtual ~hgeGUIText(){}
	hgeGUIText(int id, float x, float y, float w, float h, hgeFont2 *fnt);
...
{
public:
	hgeGUIListbox(int id, float x, float y, float w, float h, hgeFont2 *fnt, DWORD tColor, DWORD thColor, DWORD hColor);
	virtual			~hgeGUIListbox();

	int				AddItem(wchar_t *item);
